...
- [x] 1.1 In `src/main.rs::main`, dispatch on the first non-flag
...
- [x] 1.2 Update `parse_args` (or add a small helper) to return
...
- [x] 2.1 Create `src/compare.rs` with a public `pub fn run(paths:
  &[PathBuf], json: bool) -> io::Result<()>`.
- [x] 2.2 Internally: for each path, load (`load`), simulate
...
- [x] 2.3 On any `load` / `into_scenario` / `run` error, return an
  `io::Error` with the path and reason; `main` prints it to stderr
  and exits with code 1.

## 3. Renderers

- [x] 3.1 Text renderer: aligned columns, two-decimal floats,
  integer months. Output is a single `String` printed once to
  stdout.
- [x] 3.2 TTY renderer: ratatui table with the same columns; one
...
- [x] 6.4 Run `ender scenario.yaml` and confirm byte-identical text
